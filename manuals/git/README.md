...
Атомарний коміт містить одну завершену зміну, яку можна пояснити, перевірити й окремо скасувати. Виправлення функції та тест на знайдену помилку належать до одного коміту. Незалежне редагування інструкції встановлення — до іншого.

Наприклад, у власному навчальному проєкті виправлено перевірку діапазону в `analyzer/analyzer.go`, додано відповідний тест і незалежно уточнено `README.md`. Спочатку перегляньте всі зміни:
...
Зупиніться, якщо тест не пройшов. Перевірте, що staged diff містить тільки виправлення та його тест, після чого створіть коміт:

```powershell
git commit -m "fix(analyzer): reject values outside allowed range"
git add -- README.md
git diff --staged
...
Очікуються два коміти з різними цілями. Назви файлів у цьому сценарії замініть на власні; команди не створюють код і тести. Якщо інших змін не було, останній `status` порожній. Перевірка тестів у робочому каталозі охоплює також незастейджений код: не залишайте там частину виправлення, без якої коміт не працює.

Якщо дві незалежні зміни потрапили в один уже відстежуваний файл, використайте `git add -p -- <file>`. В інтерактивному виборі `y` додає фрагмент, `n` пропускає, `s` розділяє його, якщо це можливо, а `q` завершує вибір. Після цього знову прочитайте `git diff --staged`. Якщо фрагменти неможливо відокремити без втрати коректності, спочатку розділіть саме редагування файла.

### Conventional Commits

У форматі `<type>[optional scope]: <description>` тип пояснює характер зміни, необов’язкова область (`scope`) — її частину, а опис — конкретний результат. Для навчальних комітів використовуйте нижній регістр типу й області, стислий наказовий опис без крапки.
...
| `chore` | обслуговування проєкту |

Несумісну зміну позначають `!` перед двокрапкою або завершальним записом `BREAKING CHANGE:`. Для навчальної роботи корисно поєднати позначку з поясненням переходу:

```text
feat(cli)!: require an explicit input file

BREAKING CHANGE: replace implicit stdin input with the --input option
```

Це текст повідомлення, а не команда оболонки. `!` означає порушення сумісності з попереднім використанням, а не просто велику чи важливу зміну. Формат допомагає читати історію, але не робить коміт атомарним автоматично. Повідомлення `update` або `fix stuff` не пояснюють результат. Правила формату наведено в [Conventional Commits 1.0.0 українською](https://www.conventionalcommits.org/uk/v1.0.0/).

### Практика з LLM і самоперевірка

Онлайн-чат доречний для пояснення поділу змін і повідомлень: доступ до проєкту не потрібний. Надайте знеособлений опис двох змін і попросіть запропонувати межі комітів та назви. Самостійно зіставте відповідь зі staged diff, виправте неточності й перевірте кожну зміну. Якщо залучаєте агента, дозвольте йому лише читати diff у навчальному каталозі; коміти в цій вправі створюйте самостійно.

У [журналі LLM](../llm-assisted-development/README.md) зазначте інструмент, модель за наявності, дату, мету, істотний запит, прийнятий результат, власні зміни та перевірку. Не передавайте секрети або закритий код.

Перевірте себе: чи має кожен коміт одну мету; чи всі потрібні файли включено; чи опис відповідає diff; чи можна скасувати одну зміну без втрати іншої? Для звіту достатньо короткого пояснення поділу, назв комітів і фактичного результату перевірки.
